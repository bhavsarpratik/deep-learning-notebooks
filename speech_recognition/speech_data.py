...
# TODO! see https://github.com/pannous/caffe-speech-recognition for some
# data sources
...
def spectro_batch_generator(batch_size=10, width=64, source_data=Source.DIGIT_SPECTROS, target=Target.digits):
    ...
    while True:
        ...
        for image_name in files:
            ...
            # classe=(ord(image_name[0]) - 48)  # -> 0=0 .. A:65-48 ... 74
            # for 'z'
...
def dense_to_one_hot(batch, batch_size, num_labels):
    ...
    sparse_to_dense = tf.sparse_to_dense(
        concatenated, output_shape, 1.0, 0.0)
...
def read_data_sets(train_dir, source_data=Source.NUMBER_IMAGES, fake_data=False, one_hot=True):
    ...
    if fake_data:
        ...
        data_sets.validation = DataSet(
            [], [], fake_data=True, one_hot=one_hot)
